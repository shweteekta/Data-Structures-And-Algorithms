package BinaryTree.BinarySearchTree;
//There is an integer array nums sorted in ascending order (with distinct values).
//        Prior to being passed to your function, nums is possibly rotated at an unknown pivot index k (1 <= k < nums.length) such that the
//        resulting array is [nums[k], nums[k+1], ..., nums[n-1], nums[0], nums[1], ..., nums[k-1]] (0-indexed). For example, [0,1,2,4,5,6,7]
//        be rotated at pivot index 3 and become [4,5,6,7,0,1,2].
//        Given the array nums after the possible rotation and an integer target, return the index of target if it is in nums, or -1 if it is not in nums.
//        You must write an algorithm with O(log n) runtime complexity.
//        Input: nums = [4,5,6,7,0,1,2], target = 0
//        Output: 4
public class SearchInRotatedArray {
    public static int search(int[] nums, int target) {
        int result = -1;
        result = getSearch(nums, target, 0, nums.length - 1);
        return result;
    }

    private static int getSearch(int[] nums, int target, int low, int high) {
        if (low > high){
            return -1;
        }
        else{
            int mid = low + (high-low) / 2;
            if (nums[low] == target) {
                return low;
            }
            if (nums[high] == target){
                return high;
            }
            if (nums[mid] == target){
                return mid;
            }
            else if ((nums[mid] > nums[low] && nums[mid] > nums[high]) || (nums[mid] < nums[low] && nums[mid] < nums[high])){
                int value = getSearch(nums, target, low, mid -1);
                return value == -1 ? getSearch(nums, target, mid + 1, high) : value;
            }
            else if (nums[mid] > target){
                return getSearch(nums,target, low, mid-1);
            }
            else {
                return getSearch(nums, target, mid + 1, high);
            }
        }
    }

    public static void main(String[] args){
        int[] nums = {3, 1};
        int target = 1;
        System.out.println(search(nums, target));
    }
}
